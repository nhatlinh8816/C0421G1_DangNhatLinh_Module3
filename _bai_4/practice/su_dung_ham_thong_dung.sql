...
select student.student_id,student.student_name,avg(mark) 
from student join mark on student.student_id = mark.student_id
group by student_id,student_name having avg(mark) >15;
select student.student_id,student.student_name,avg(mark) 
from student join mark on student.student_id = mark.student_id
group by student_id,student_name having avg(mark) >=all(select avg(mark) from mark group by mark.student_id);