...
select order_id,customer_customer_id,order_date from `order`;
select customer.customer_id,customer.customer_name,product.product_id,product.product_name
from customer join `order` on customer.customer_id = `order`.customer_customer_id join order_detail on  `order`.order_id = orderdetail.order_order_id
join product on order_detail.product_product_id = product.product_id;
select * from customer where customer_id not in (select customer_customer_id from `order`);
select order_detail.order_order_id,`order`.order_date,sum(order_detail_qty*product.product_price) as order_total_price  
from `order` join order_detail  on `order`.order_id = order_detail.order_order_id
join product on order_detail.product_product_id = product.product_id
